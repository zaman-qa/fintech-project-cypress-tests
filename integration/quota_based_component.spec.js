...
describe('Creating Quota based Component', () => {
  let sharedData = new sharedContent()
  let loginMethods = new loginMethod()
  let operation = 'sum of amounts'
  let accelerationMethod = 'incremental'
  let componentNameQMeasure = 'QB-CommulativeM-' + sharedData.randomString(2)
  let componentNameAddFilters = 'QB-AddFilters-' + sharedData.randomString(2)
  ...
  describe('Simple Component with different Cumulative Quota Measures', () => {
    it('Create with Sum of Amount', () => {
      createComponent(operation)
    })

    it('Update to Count of Activities', () => {
      operation = 'count of activities'
      createComponent(operation)
    })
  })

  describe('Two Tiers with different acceleration methods', () => {
    it('Create with two Tiers and Incremental Acceleration Method', () => {
      createComponentWithTwoTiers(accelerationMethod)
    })

    it('Create with two Tiers and Retroactive Acceleration Method', () => {
      accelerationMethod = 'retroactive'
      createComponentWithTwoTiers(accelerationMethod)
    })
  })
  ...
  function createComponent(operation) {
    sharedData.goToPlans(planName)
    if (operation === 'sum of amounts') {
      ...
      cy.get('.active').contains('Step 1').then(() => {
        cy.get('#component-name').type(componentNameQMeasure).should('have.value', componentNameQMeasure)
        cy.get('.fa-check').should('be.visible')
        cy.get('.custom-radio').contains('QUOTA BASED').click()
      ...
      cy.get('#form-step-1', { timeout: 10000 }).should('be.visible').then(($step2) => {
        $step2.find('.ss-single-selected')[0].click({ force: true })
        cy.get('.ss-option').contains(mainTable).click({ force: true })

        cy.get('.placeholder').contains('Select Owner').click({ force: true })
        cy.get('.ss-option').contains('the payee').click({ force: true })
        cy.get('.placeholder').contains('Select Field', { timeout: 10000 }).should('be.visible').click({ force: true })
        cy.get('.ss-option').contains('Opp Owner').click({ force: true })

        cy.get('.placeholder').contains('Select Credit Earned Date').click({ force: true })
        cy.get('.ss-open').find('.ss-option').contains('Close Date').click({ force: true })

        cy.get('.placeholder').contains('Select Stage Field').click({ force: true })
        cy.get('.ss-open').find('.ss-option').contains("I don't have a stage field").click({ force: true })
        cy.get('#component-next-button', { timeout: 5000 }).click()
      })

      cy.get('.placeholder').contains('Select Operation').click({ force: true })
      cy.get('.ss-open').contains(operation).click()

      cy.get('.quota-attainment').find('.placeholder').contains('Select Field').click({ force: true })
      cy.get('.ss-open').not('div[style="display: none;"]').contains('Amount').click()

      cy.get('.placeholder').contains('Select Time Period').click({ force: true })
      cy.get('.ss-open').find('.ss-option').eq(2).click()

      cy.get('#default-quota-check').check({ force: true })
      cy.get('#default_quota').clear().type('10')
      ...
      cy.get('td').contains(componentNameQMeasure).click()
      cy.get('#component-next-button', { timeout: 5000 }, { timeout: 10000 }).should('be.visible').click()
      cy.get('#component-next-button', { timeout: 5000 }, { timeout: 10000 }).should('be.visible').click()
      cy.get('.placeholder').contains('with a sum of amounts').click({ force: true })
      cy.get('.ss-open').contains(operation).click()
      cy.get('.quota-attainment').find('.placeholder').contains('Select Field').click({ force: true })
      cy.get('.ss-open').find('.ss-option').eq(1).click()
      cy.get('#component-next-button', { timeout: 5000 }).should('be.visible').click()
    ...
    cy.get('#components').then(($components) => {
      if ($components.find('#quota-main-div', { timeout: 250000 }).length > 0) {
        ...
        cy.get('#plan-components-table', { timeout: 10000 }).should('be.visible').contains(`${componentNameQMeasure}`).should('exist')
      }
      else {
        cy.get('#component-prev-button').click({ force: true })
        cy.get('#component-next-button', { timeout: 5000 }).should('be.visible').click()
        cy.wait(5000)
        cy.get('#quota-main-div', { timeout: 10000 })
        cy.get('#component-finish-button').click()
        cy.get('#plan-components-table', { timeout: 10000 }).should('be.visible').contains(`${componentNameQMeasure}`).should('exist')
      }
    })
  ...
  function createComponentWithTwoTiers(accelerationMethod) {
    sharedData.goToPlans(planName)
    let componentNameTwoTiers = 'QB-TwoTiers-' + sharedData.randomString(2)
    cy.get('#add-component-btn', { timeout: 10000 }).should('be.visible').click()
    cy.get('#component-steps', { timeout: 30000 }).should('be.visible')

    cy.get('.active').contains('Step 1').then(() => {
      cy.get('#component-name').type(componentNameTwoTiers).should('have.value', componentNameTwoTiers)
      cy.get('.fa-check').should('be.visible')
      cy.get('.custom-radio').contains('QUOTA BASED').click()
      cy.get('#component-next-button', { timeout: 5000 }).should('be.visible').click()
    })
    ...
    cy.get('#form-step-1', { timeout: 10000 }).should('be.visible').then(($step2) => {
      ...
      cy.get('.placeholder').contains('Select Stage Field').click({ force: true })
      cy.get('.ss-open').find('.ss-option').contains("I don't have a stage field").click({ force: true })
      cy.get('#component-next-button', { timeout: 5000 }).click({ force: true })
    })

    cy.get('.placeholder').contains('Select Operation').click({ force: true })
    cy.get('.ss-open').contains(operation).click({ force: true })

    cy.get('.quota-attainment').find('.placeholder').contains('Select Field').click({ force: true })
    cy.get('.ss-open').not('div[style="display: none;"]').contains('Amount').click({ force: true })

    cy.get('.placeholder').contains('Select Time Period').click({ force: true })
    cy.get('.ss-open').find('.ss-option').eq(2).click({ force: true })

    cy.get('.max-value').last().type('100')
    cy.get('#add-accelerator-tier').click()
    cy.get('.base-rate').last().clear().type(2.0)
    cy.get('.accelerator-method').last().select(accelerationMethod, { force: true })
    ...
    cy.get('#components').then(($components) => {
      if ($components.find('#quota-main-div', { timeout: 250000 }).length > 0) {
        cy.get('#component-finish-button').click()
        cy.get('#plan-components-table', { timeout: 10000 }).should('be.visible').contains(`${componentNameTwoTiers}`).should('exist')
      }
      else {
        cy.get('#component-prev-button').click({ force: true })
        cy.get('#component-next-button', { timeout: 5000 }).should('be.visible').click()
  ...
  function createComponentWithAdditionalFilters(operator) {
    ...
    if (operator === 'is equal to') {
      ...
      cy.get('#form-step-1', { timeout: 10000 }).should('be.visible').then(($step2) => {
        ...
        cy.get('div.credit-rules-condition-fields').last().click()
        cy.get('.ss-open').find('.ss-option').contains('Amount').click()

        cy.get('div.credit-rules-condition-operators').last().click()
        cy.get('.ss-open').find('.ss-option').contains(operator).click()

        if (operator != ('is blank') && operator != ('is not blank'))
          cy.get('input.filter-condition-value').last().type('123')
  ...
  function createComponentWithReleaseMethodology(payPeriodMethod, releaseMethodology, componentName, conditionalOperator) {
    ...
    if (payPeriodMethod === 'Standard') {
      ...
      cy.get('div.pay-period-method').last().click()
      cy.get('.ss-open').find('.ss-option').contains(payPeriodMethod).click({ force: true })

      cy.get('div.payout-release-methodology').last().click()
      cy.get('.ss-open').find('.ss-option').contains(releaseMethodology).click({ force: true })
  ...
  function getComponentName(releaseMethodology) {
    let componentName = ""
    if (releaseMethodology === 'Full release, based on a condition')
      componentName = 'QB-FR-condition-' + sharedData.randomString(2);
    else if (releaseMethodology === 'Full release, based on a date')
      componentName = 'QB-FR-date-' + sharedData.randomString(2);
    else if (releaseMethodology === 'Proportional release, based on a date')
      componentName = 'QB-PR-date-' + sharedData.randomString(2);
    else if (releaseMethodology === 'Proportional release, based on a frequency')
      componentName = 'QB-PR-frequency-' + sharedData.randomString(2);
